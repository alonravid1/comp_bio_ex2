...
class DarwinAlgo(GeneticAlgo):
    ...
    def optimization_score(self, solution):
        """Darwin optimizatoin function - make a swap in the
        solution and check its new score. If it is higher than its current
        score, take the new higher one. repeat for several swaps, each time comparing
        the new score to the best so far.

        Args:
            solution (np.array): _description_

        Returns:
            float: best score for a given swap
        """
    ...
    def evolve_new_gen(self, solutions):
        """calculate the optimized score of the previous gen, sort its solutions
